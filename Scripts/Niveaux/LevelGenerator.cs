...
namespace SpaceZombie.Niveaux.Configs.V
{
    ...
    public class LevelGenerator
    {
        ...
        private NiveauZombiesSpawnSettings GenererNiveauService(Level setting, int stage)
        {
            ...
            for (int i = 0; i < ligneSettings.Length; i++)
            {
                if (i < NB_LIGNES_VISIBLE)
                {
                    ...
                    ligneSettings[i] = CreerLigne(ligne.Speed, ligne.Fixed, ligne.Direction, setting.NbEnemiesParLigne, EnemyConfig.Mapp(ligne.EnemyLevels[0]));
                }
                else
                {
        ...
        #region Commun
        private NiveauLigneSettings CreerLigne(float vitesseLigne, bool fixe, float directionX, int nbEnemies, NiveauEnemySettings enemyTypeMapp)
        {
            return new NiveauLigneSettings()
            {
                EnemySlotSettings = CreerNiveauEnemySlotSettings(nbEnemies, true, enemyTypeMapp),
        ...
        private NiveauLigneSettings CreerLigneNonVisible()
        {
            return new NiveauLigneSettings()
            {
                EnemySlotSettings = CreerNiveauEnemySlotSettings(1, false, Enemies.Configs.EnemyConfig.enemyType1),
                Vitesse = GlobalMinSpeed,
                IsFixe = true,
                DirectionX = DIRECTION_DEFAUT
            };
        }
        private NiveauEnemySlotSettings[] CreerNiveauEnemySlotSettings(int length, bool isVisible, NiveauEnemySettings enemy)
        {
            NiveauEnemySlotSettings[] slots = new NiveauEnemySlotSettings[length];
            for (int i = 0; i < length; i++)
            {
                slots[i] = CreerNiveauEnemySlotSettings(isVisible, enemy);
            }
            return slots;
        }
        private NiveauEnemySlotSettings CreerNiveauEnemySlotSettings(bool isVisible, NiveauEnemySettings enemy)
        {
            return new NiveauEnemySlotSettings() { enemyObjSettings = CreerNiveauEnemyObjSettings(isVisible, enemy) };
        }
        private NiveauEnemyObjSettings CreerNiveauEnemyObjSettings(bool isVisible, NiveauEnemySettings enemy)
        {
            return new NiveauEnemyObjSettings()
            {
                IsVisible = isVisible,
                enemySettings = CreerNiveauEnemySettings(enemy)
            };
        }
        private NiveauEnemySettings CreerNiveauEnemySettings(NiveauEnemySettings enemy)
        {
            return new NiveauEnemySettings()
            {
                EnemyObj = enemy.EnemyObj,
                Enemy = new Enemies.Enemy(enemy.Enemy.Hp, enemy.Enemy.Score)
            };
        }
        #endregion
    }
